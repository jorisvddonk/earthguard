define('starmap', ["simproperties", "star", "namegen"], function(SIMPROPS, Star, Phonetics){
  ...
  Starmap.prototype.generateStarmap = function() {
    ...
    var reject_count = 0;
    while (this.stars.length < SIMPROPS.NSTARS && reject_count < SIMPROPS.MAX_NUMBER_OF_STAR_REJECTS) {
  ...
  Starmap.prototype.getClosestStars = function(cstar) {
    var dists = _.map(_.without(this.stars, cstar), function(star) {
        return {"star": star, "dist": Math.pow(cstar.mapx-star.mapx,2) + Math.pow(cstar.mapy-star.mapy,2)};
    });
  ...
  Starmap.prototype.getShortestpath = function(srcstar, deststar) {
    ...
    while (openset.length > 0) {
      ...
      openset = _.filter(openset, function(sta){return sta.objid != current.objid});
      closedset.push(current);
      var neighbors = this.getLinkedStars(current);
      for (var i = 0; i < neighbors.length; i++) {
        var neighbor = neighbors[i];
        var tentative_g_score = dist_between(current, neighbor) + (g_score[current.objid] === undefined ? 0 : g_score[current.objid]);
        ...
        if (!_.contains(openset, neighbor) || tentative_g_score < g_score[neighbor.objid]) {
