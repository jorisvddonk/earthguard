define('namegen', function(){
  ...
  */
  ...
  var Phonetics = function Phonetics() {
    ...
    this.chargroups.remainingconsonants = {"key": "r", "values": _.difference(this.chargroups["consonants"]["values"],     this.chargroups["affricates"]["values"],this.chargroups["fricatives"]["values"],this.chargroups["stops"]["values"],this.chargroups["nasals"]["values"],this.chargroups["liquids"]["values"],this.chargroups["glides"]["values"])};
      ...
      if (next_is_literal) {
        ...
        for (var pkey in this.chargroups) {
          if (pl == this.chargroups[pkey]["key"]) {
            genChar = this.chargroups[pkey]["values"][_.random(0, this.chargroups[pkey]["values"].length-1)];
          }
        }
      }
